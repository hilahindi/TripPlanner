...
- Day 2 is from B to C and should also be up to 60km
- The total distance should be around 120km max
All points must be connected by real cycling paths supported by OpenRouteService.
...
// Using Groq API for AI model interaction
  const res = await fetch('https://api.groq.com/openai/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        { role: 'system', content: 'You help plan realistic cycling trips using GPS coordinates.' },
...
// This function will attempt to fetch a hiking route up to 5 times before failing
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      console.log(`Attempt ${attempt}: Requesting points from AI...`);
      const [a, b, c] = await getCyclingPointsFromAI(city);

      console.log(`Points received. Fetching routes A→B and B→C...`);
      const day1 = await fetchRoute(a, b);
      const day2 = await fetchRoute(b, c);
      ...
      if (attempt === maxAttempts) {
        throw new Error('Failed to generate cycling route after 5 attempts.');
      }
    }
  }
}
